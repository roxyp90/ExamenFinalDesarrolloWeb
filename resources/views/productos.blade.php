                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4 col-sm-6 col-12">
                                    <div class="card"
                                        style="border: 2px solid #E8C547; border-radius: 8px; background: white;">
                                        <div class="card-header"
                                            style="background-color: #fef9f0; border-color: #E8C547;">
                                        ...
                                        <div class="card-footer"
                                            style="background-color: #faf9f8; border-top: 1px solid #E8C547;">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content" style="background-color: white; border: 1px solid #E8C547;">
            <div class="modal-header" style="background-color: #fafafa; border-bottom: 2px solid #E8C547;">
                <h5 class="modal-title" id="modalAgregarProductoLabel" style="color: #D97A26; font-weight: bold;">
                    Agregar Nuevo Producto</h5>
            ...
            <div class="modal-body">
                <form>
                    <div class="form-group">
                        <label for="nombreProducto" style="color: #4A2E14; font-weight: 600;">Nombre del
                            Producto</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control" id="precioProducto" placeholder="0.00"
                            style="border: 1px solid #E8C547; border-radius: 4px;">
                    </div>
                    <div class="form-group">
                        <label for="stockProducto" style="color: #4A2E14; font-weight: 600;">Stock Inicial</label>
                        <input type="number" class="form-control" id="stockProducto" placeholder="0"
                            style="border: 1px solid #E8C547; border-radius: 4px;">
                    </div>
                    <div class="form-group">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        btnGuardarProducto.addEventListener('click', function() {
            ...
            if (editingCard) {
                // Modo edición
                const cardHeader = editingCard.querySelector('.card-header .card-title');
                const cardBody = editingCard.querySelector('.card-body');

                cardHeader.textContent = nombre;
                cardBody.innerHTML = `
                    <p class="card-text">${descripcion}</p>
                    <p style="color: #688B3E; font-weight: bold;">$${precio.toFixed(2)}</p>
                    <p style="color: #999;">Stock: ${stock} unidades</p>
                `;

                editingCard = null;
                btnGuardarProducto.textContent = 'Guardar Producto';
                document.getElementById('modalAgregarProductoLabel').textContent = 'Agregar Nuevo Producto';
            } else {
                // Modo agregar
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('btn-editar-producto') || e.target.textContent === 'Editar') {
                ...
                const precioText = card.querySelector('p[style*="color: #688B3E"]').textContent;
                const precio = parseFloat(precioText.replace('$', ''));
                const stockText = card.querySelector('p[style*="color: #999"]').textContent;
                const stock = parseInt(stockText.replace('Stock: ', '').replace(' unidades', ''));
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('btn-eliminar-producto') || e.target.textContent ===
                'Eliminar') {
                if (confirm('¿Está seguro de que desea eliminar este producto?')) {
                    const cardContainer = e.target.closest('.col-md-4, .col-sm-6, .col-12');
